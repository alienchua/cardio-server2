...
`node --experimental-test-coverage --test test/*.test.js` passed 28 tests. Repository-wide line coverage is 17.55% because the legacy model modules are mostly outside the current test suite; the focused changed query path executed successfully. No live authenticated browser assertion was possible because the available check-in session showed the admin login page.
